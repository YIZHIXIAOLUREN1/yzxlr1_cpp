...
int main(){
    ...
    while(T--){
        ...
        //dp[n]=h[n];
        for(int i=n;i>=1;i--){
            dp[i]=min(query1(0,1,1,n,i)+i+2-s[i-1],query1(1,1,1,n,i)+2*i+2-s[i-1]);
            dp[i]=min(dp[i],s[n]-s[i-1]-(n-i));
            //printf("%d %d\n",dp[i]-h[i]+h[i-1]+s[i-2]-i,dp[i]+h[i-1]+s[i-2]-2*i);//
            update1(0,1,1,n,1,i-h[i],dp[i]-h[i]+h[i-1]+s[i-2]-i);
            update1(1,1,1,n,max(i-h[i],1),i-2,dp[i]+h[i-1]+s[i-2]-2*i);
            //cout<<dp[i]<<":\n";//
            for(int j=1;j<i;j++){
            	//cout<<query1(0,1,1,n,j)<<" \n"[j==i-1];//
            }
            for(int j=1;j<i;j++){
            	//cout<<query1(1,1,1,n,j)<<" \n"[j==i-1];//
            }
        }
        cout<<dp[1]<<"\n";
    }
