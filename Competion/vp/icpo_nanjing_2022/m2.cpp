...
Point P[N];
int ans;
#define li(x) (x>1?x-1:n)
#define ri(x) (x<n?x+1:1)
bool vis[N];
int main(){
    ...
    for(int i=1;i<=n;i++){
        if(P[li(i)].y<P[i].y||P[ri(i)].y<P[i].y||vis[i])continue;
        else if(round(P[li(i)].y)==round(P[i].y)||round(P[ri(i)].y)==round(P[i].y)){
            int l=i,r=i;
            while(round(P[li(l)].y)==round(P[l].y))l=li(l);
            while(round(P[ri(r)].y)==round(P[r].y))r=ri(r);
            ...
            che=che*(1/(2*Len(che)));
            che=che+pz;
            if(P[li(l)].y>P[i].y&&P[ri(r)].y>P[i].y&&PIP(P,n,che)==1){
                ans++;
            }
            //printf("%d:%d %d\n",i,l,r);
        }else{
